...
def generate_url(
    path: str = "", params: dict | None = None, queryParams: dict | None = None
):
    """
...
def get_schedule_ical(group_id: str, start_date: str, end_date: str) -> str:
    ...
    schedules_set_date_range_url = generate_url(
        SCHEDULES_SET_DATE_RANGE_URL, {"id": group_id}
    )
    ...
    schedules_ical_url = generate_url(
        ICAL_URL, {"id": group_id}, {"dO": start_date, "dD": end_date}
    )
    ...
    with requests.Session() as session:
        ...
        response = session.post(
            schedules_set_date_range_url, data=schedules_set_date_range_data
        )
        ...
        print(
            f"Fetched schedule for group with id {group_id} from {start_date} to {end_date}"
        )
